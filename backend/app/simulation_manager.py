...
class SimulationManager:
    ...
    def __init__(self, data_provider: BaseDataProvider, base_interval: float = 1.0, mqtt_service: Any = None):
        ...
        self.speed: float = 60.0  # Default 60x speed (1 sim minute = 1 real sec)
    ...
    def _interrupt_worker_nolock(self):
        """Interrupts and stops current worker thread if running."""
    ...
    def _worker_loop(self):
        """Background thread loop advancing simulation records smoothly according to self.speed."""
        while not self._stop_event.is_set():
            if self.state == SimulationState.RUNNING:
                ...
                # Dynamic step delay based on speedup multiplier
                # Default 60x speed: 1 row (1 min sim) per 1s real time
